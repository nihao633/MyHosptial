...
<div class="row my-3 mx-0 mx-sm-5 d-block d-xl-flex">
    ...
    <div class="col-12 col-lg-8 col-xl-6 mb-3">
        ...
        <div class="setting-box form-control mb-5">
            <div class="mb-3">
            ...
            <SubHeader @create_mode="set_create_mode" @click_two="show_set_date" :button_name_two="(selected_consultant == '') ? '' : 'Set Consultant Date'" :route_path="'#create_dialog'" :title="'Consultants:'" :button_name="'Add Consultant'">
                <ListView :loading="content_loading" :array="consultants" :selected_value="selected_consultant" @select="select_consultant" @edit_on="edit_on" />
            </SubHeader>
            <SubHeader :route_path="'/register'" :title="'Users:'" :button_name="'Add User'">
                <ListView :loading="content_loading" :array="users" :selected_value="selected_user" @select="select_user" :disabled_edit=true />
            </SubHeader>
            <SubHeader @create_mode="set_create_mode" :route_path="'#create_dialog'" :title="'Branches:'" :button_name="'Add Branch'">
                <ListView :loading="content_loading" :array="branches" :selected_value="selected_branch" @select="select_branch" @edit_on="edit_on" />
            </SubHeader>
            ...
            <div id="#select_date" class="modal" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Set Appointment Date for {{ selected_consultant.name }}</h5>
...
onMounted(() => {
    ...
    $('#edit_dialog').on('hide.bs.modal', function (e) {
        initiate_settings()
    })
    $('#confirm_dialog').on('hide.bs.modal', function (e) {
        initiate_settings()
    })
    $('#create_dialog').on('hide.bs.modal', function (e) {
        initiate_settings()
    })
    initiate_settings()
})
...
const show_set_date = () => $('#select_date').modal('toggle')
</script>
